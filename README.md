...
- Current trained SVM models will only be accurate if the data has been base-called with Albacore 2.1.7. We are working on training new models for Guppy base-called data, as well as working on improving the models by using additional features. If you are using Guppy base-called fast5/fastq, you can still use EpiNano to extract features (i.e. 'errors', but the SVM predictions (ProbM) will not be accurate. 
